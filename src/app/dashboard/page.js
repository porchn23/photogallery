  ...
  async function fetchInitialData() {
    ...
    try {
      ...
      if (userData) {
        ...
        // รวมและจัดเรียง (งานตัวเองขึ้นก่อน)
        const allEvents = [...ownedEvents, ...joinedEvents]
          .filter((v, i, a) => a.findIndex(t => t.id === v.id) === i) // กันเหนียวลบตัวซ้ำ
...
const handleJoinEvent = async (e) => {
  ...
  try {
    ...
    // 2. ถ้าเป็นเจ้าของเอง ให้ผ่านไปหน้างานเลย
    if (eventData.owner_id === user.id) {
      router.push(`/dashboard/event/${eventData.id}`);
      return;
    }

    // 3. ตรวจสอบสมาชิก (ใช้ maybeSingle เพื่อไม่ให้ Error ถ้าไม่พบ)
    ...
    // 4. ถ้ายังไม่เป็นสมาชิก ให้เพิ่มชื่อเข้าทีม
    if (!existing) {
    ...
    // 5. เมื่อจอยสำเร็จ หรือเป็นสมาชิกอยู่แล้ว ให้พาไปหน้าจัดการงาน
    setIsJoining(false);
    setJoinCodeInput('');
    router.push(`/dashboard/event/${eventData.id}`);
...
// ... existing code ...
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-[#09090b] text-black dark:text-white font-sans">
      ...
      {/* Modals (Create & Join) - เหมือนเดิมตามที่คุณตกลง */}
      {isCreating && (
          ...
          <div className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-[3rem] p-10 md:p-12 shadow-2xl border border-zinc-100 dark:border-zinc-800">
            <div className="text-center mb-10">
              ...
              <div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-3xl p-6 text-left space-y-3 mb-8">
                ...
                <div className="pt-2 border-t border-zinc-200 dark:border-zinc-700 mt-2"><p className="text-[10px] text-zinc-500 italic">* สามารถซื้อ Slot และเพิ่มวันเก็บรูปได้ในภายหลัง</p></div>
              </div>
            </div>
